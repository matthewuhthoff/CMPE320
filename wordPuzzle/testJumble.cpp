...
void testJumble() {
	...
	try {
		JumblePuzzle("ow", "easy"); // word too short
	} catch (BadJumbleException& err){
		std::cout << err.what();
	}


	try {
		JumblePuzzle("supercalifragilisticexpialidocious", "easy"); // word too long
	} catch (BadJumbleException& err){
		std::cout << err.what();
	}

	try {
		JumblePuzzle("matthew", "mild"); // invalid difficulty
	} catch (BadJumbleException& err){
		std::cout << err.what();
	}
